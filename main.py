...
if data_load:
    ...
    noise1_origin, __sr__ = librosa.load('../datasets/demand/DKITCHEN/ch01.wav', sr=16000)
    noise2_origin, __sr__ = librosa.load('../datasets/demand/NRIVER/ch01.wav', sr=16000)
    noise3_origin, __sr__ = librosa.load('../datasets/demand/NFIELD/ch01.wav', sr=16000)
    noise4_origin, __sr__ = librosa.load('../datasets/demand/OOFFICE/ch01.wav', sr=16000)

    for i in range(5500):
        wave, __sr__ = librosa.load(file_name[i], sr=16000)
        wave = wave[:512 * 300]  # y_data
        random_sample = random.randint(500, 160000)

        noise1 = np.expand_dims(noise1_origin[random_sample:random_sample + 512 * 300], 0)
        noise2 = np.expand_dims(noise2_origin[random_sample:random_sample + 512 * 300], 0)
        noise3 = np.expand_dims(noise3_origin[random_sample:random_sample + 512 * 300], 0)
        noise4 = np.expand_dims(noise4_origin[random_sample:random_sample + 512 * 300], 0)
        ...
        wave_temp = np.copy(wave)
        random_scale = random.sample(range(-10, 11), 4)
        scale1 = personal.adjust_snr(wave_temp, noise1, random_scale[0])
        scale2 = personal.adjust_snr(wave_temp, noise2, random_scale[1])
        scale3 = personal.adjust_snr(wave_temp, noise3, random_scale[2])
        scale4 = personal.adjust_snr(wave_temp, noise4, random_scale[3])
...
dataset = custom_datasets.CustomDataset(train=True, path='../datasets/NSDTSEA')
dataloader = torch.utils.data.DataLoader(dataset=dataset, batch_size=batch_size, shuffle=True, drop_last=False)
dataset_test = custom_datasets.CustomDataset(train=False, path='../datasets/NSDTSEA')
...
def evaluate_u(data_loader, denoise_func):  # evaluate
    ...
    with torch.no_grad():
        for batch_idx, (x_data, y_data) in enumerate(data_loader):
            ...
            # for index, gan_data in enumerate(pred):
            #     y_max = np.max(y_data[index])
            #     pred_max = np.max(pred[index])
            #     sf.write('../datasets/libriCustom/test/eval_data/' + str(gan_index) + '.wav',
            #              gan_data * y_max / pred_max, 16000)
            #     print(gan_index, end=' ')
            #     gan_index += 1
...
if train_u_bool:
    ...
    torch.save(_model.state_dict(), './saved_models/giant_model8.pt')

    print("Done!")

if evaluate:
    if not train_u_bool:
        state_dict = torch.load('./saved_models/giant_model8.pt', map_location=device)
